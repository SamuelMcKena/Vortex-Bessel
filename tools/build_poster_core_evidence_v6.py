"""Poster core evidence v6.

The v5 full-XY benchmark showed that axicon decentre could still be estimated but
beam pointing remained strongly confounded in this intensity-only configuration.
That is scientifically useful: the inference layer must screen identifiability
rather than emit every optimizer minimum as a physical fact.

This pass therefore benchmarks a more separable pair of physical planes:
axicon lateral decentre and 4F iris offset.  The truth is simultaneous, off-grid
and noisy, and the complete cropped XY intensity stack over 17 z planes is used
in the objective.
"""
...
OUT = ROOT / "outputs" / "poster" / "core_evidence_v6"


def build_joint_parameter_recovery_axicon_iris(out: Path, *, grid_n: int = 256) -> tuple[Path, dict, Path]:
    ...
    truth_clean, coords = v5._xy_stack(
        config(truth_dec_um, truth_iris_r),
        grid_n=grid_n,
        z_m=z,
        halfwidth_m=halfwidth_m,
        label="poster-v6-truth",
    )
    ...
    def simulate(dec_um: float, iris_r: float) -> np.ndarray:
        return v5._xy_stack(
            ...
            halfwidth_m=halfwidth_m, label=f"poster-v6-{dec_um:g}-{iris_r:g}",
        )[0]
    ...
    best, _ = v5._xy_stack(
        ...
        halfwidth_m=halfwidth_m, label="poster-v6-best",
    )
    ...
    lines = [
        ...
        ("Injected 4F iris offset", f"{truth_iris_r:+.2f} R"),
        ("Recovered 4F iris offset", f"{fine.best_y:+.2f} R"),
    ...
    fig.suptitle("Recovering two physical errors from the simulated camera stack",
                 color=v4.FG, fontsize=18.2, weight="bold", y=0.985)
    fig.text(0.5, 0.944,
             "Off-grid simultaneous axicon and 4F errors + deterministic noise; candidates are replayed through the same dual-SLM → 4F → axicon forward model.",
             ha="center", color=v4.MUTED, fontsize=9.6)

    png = v4._save(fig, out / "02_joint_axicon_iris_recovery_fullxy.png")
    summary = {
        "benchmark_scope": "synthetic model-to-model validation of simultaneous axicon decentre and 4F iris offset using full XY intensity at 17 z planes",
        "grid_n": int(grid_n),
        "z_planes": int(len(z)),
        ...
        "identifiability_note": "v5 axicon+beam-pointing test was strongly confounded; this v6 pair was chosen to test whether physically distinct error planes can be separated by the same intensity-stack objective",
    ...
    csv_path = out / "02_joint_axicon_iris_recovery_fullxy_summary.csv"
...
def contact_sheet(out: Path, paths: list[Path]) -> Path:
    ...
    for i, p in enumerate(paths[:6]):
        ...
        ax.axis("off")
    fig.suptitle("Poster core evidence v6 — self-healing + physical metrics + experimental phase retrieval",
                 color="white", fontsize=20, weight="bold", y=0.995)
    return v4._save(fig, out / "00_core_evidence_v6_contact_sheet.png", dpi=180)


def main() -> None:
    OUT.mkdir(parents=True, exist_ok=True)
    p1, m1 = v4.build_self_healing_path(OUT)
    p2, m2, csv2 = build_joint_parameter_recovery_axicon_iris(OUT)
    exp = v4.copy_experimental(OUT)
    sheet = contact_sheet(OUT, [p1, p2] + exp)
    manifest = {
        "outcome": "POSTER-CORE-EVIDENCE-V6",
        "story": "physical simulation -> self-healing -> interpretable physical-error metrics -> experimental residual-phase retrieval",
        "self_healing": m1,
        "joint_axicon_iris_recovery_fullxy": m2,
