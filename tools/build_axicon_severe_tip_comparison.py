"""Severe localised rounded-axicon comparison: ordinary B0 versus annular ell=0.

This presentation stress test intentionally models a *local* severely blunted
apex.  Inside an 800 um radial extent the sharp cone is replaced by a smooth
parabolic cap that is flat on axis and matches both the height and slope of the
sharp cone at r = 800 um.  Outside that radius the optical surface is exactly
the sharp axicon.  This is the geometry needed to test the physical question:
what happens when an ordinary beam illuminates a bad central apex versus when a
hollow non-vortex beam avoids that same defective region?

The local cap is an illustrative severe-defect model, not a metrology-derived
surface for the laboratory axicon.  The annular field is likewise an
axicon-plane planning target, not a calibrated phase-only SLM command.

Only two presentation files are emitted:
  * 09_severe_tip_avoidance_xy_xz.png
  * 09b_severe_tip_avoidance_1D.png
"""
...
Z_REF_M = 60e-3
TRANSITION_M = 45e-6
CANDIDATE_CLEAR_RADII_UM = (820, 840, 860, 880, 900, 950, 1000)
...
def _hollow_clear_target(field_b0: np.ndarray, grid, clear_radius_m: float) -> np.ndarray:
    ...
    # Equal total power at the axicon plane for a fair optical comparison.
...
def _localised_round_transmission(
    sharp_t: np.ndarray,
    grid,
    *,
    wavelength_m: float,
    ...
    """Replace only the central cone by a C1-matched parabolic rounded cap.

    Sharp sag is z_s = m r.  For r <= R_t use

        z_c = m R_t/2 + m r^2/(2 R_t),

    which has dz_c/dr = 0 on axis and matches both z_s and dz_s/dr at R_t.
    Thus the rounded-tip phase defect is strictly local: outside R_t the
    transmission is exactly the sharp-axicon transmission.
    """
    X = np.asarray(grid["X"], float)
    Y = np.asarray(grid["Y"], float)
    R = np.hypot(X, Y)
    rt = float(tip_radius_m)
    slope = math.tan(float(base_angle_rad))
    delta_sag = np.zeros_like(R, dtype=float)
    inside = R <= rt
    delta_sag[inside] = slope * (rt - R[inside]) ** 2 / (2.0 * rt)
    k0 = 2.0 * np.pi / float(wavelength_m)
    defect_phase = -k0 * (float(refractive_index) - float(external_index)) * delta_sag
...
def _build_fields(grid_n: int, clear_radius_um: float | None):
    ...
    round_t = _localised_round_transmission(
        sharp_t,
        grid,
        ...
        tip_radius_m=SEVERE_TIP_RADIUS_M,
    )
...
def select_clear_radius(grid_n: int) -> dict:
    ...
    b0_sharp_xz, _ = base._xz_from_post_axicon(b0 * sharp_t, grid, wavelength, coord, "local-severe-b0-sharp")
    b0_round_xz, _ = base._xz_from_post_axicon(b0 * round_t, grid, wavelength, coord, "local-severe-b0-round")
    ...
    for radius_um in CANDIDATE_CLEAR_RADII_UM:
        ...
        h_sharp_xz, _ = base._xz_from_post_axicon(hollow * sharp_t, grid, wavelength, coord, f"local-severe-{radius_um}-sharp")
        h_round_xz, _ = base._xz_from_post_axicon(hollow * round_t, grid, wavelength, coord, f"local-severe-{radius_um}-round")
    ...
    # Prefer the smallest practical annulus that suppresses at least 90% of the
    # local-tip modulation while preserving a bright, useful J0-like output.
    passing = [
        r for r in rows
        if r["tip_fraction"] <= 5e-4
        and r["developed_onaxis_rms_rounded_vs_sharp"] <= 0.10 * b0_rms
        and r["mean_sharp_onaxis_vs_B0"] >= 0.50
        and r["z60_centre_to_peak_rounded"] >= 0.80
    ]
    if passing:
        selected = min(passing, key=lambda r: r["clear_radius_um"])
        mode = "smallest_radius_with_ge90pct_tip_modulation_suppression"
    else:
        admissible = [r for r in rows if r["tip_fraction"] <= 5e-4 and r["z60_centre_to_peak_rounded"] >= 0.80]
        selected = min(admissible or rows, key=lambda r: (r["developed_onaxis_rms_rounded_vs_sharp"], r["clear_radius_um"]))
        mode = "best_available_local-tip_suppression"

    print("Local severe-tip sweep (800 um radial defect):")
...
def render_final(out: Path, grid_n: int, selected_um: int) -> None:
    ...
    for label, incident in (
        ("ordinary B0 through localised severe rounded tip", b0),
        (f"annular ell=0 tip avoidance, clear radius {selected_um} um", hollow),
    ):
        xy_round = base._xy_from_post_axicon(incident * round_t, grid, wavelength)
        xz_round, _ = base._xz_from_post_axicon(incident * round_t, grid, wavelength, coord, f"local-severe-final-{label}-round")
        xz_sharp, _ = base._xz_from_post_axicon(incident * sharp_t, grid, wavelength, coord, f"local-severe-final-{label}-sharp")
    ...
    for col, case in enumerate(cases):
        ...
        axes[0, col].text(
            0.03, 0.04,
            f"incident power in defective r < 0.8 mm: {100*case['tip_fraction']:.3f}%",
    ...
    fig.suptitle("Localised severe rounded apex (r = 0.8 mm): direct illumination vs annular avoidance", color=style.TEXT, fontsize=16, y=1.02)
    fig.text(0.5, -0.01, "Outside r = 0.8 mm the rounded and sharp axicon surfaces are identical. Both inputs have equal pre-axicon power; annular field is non-vortex (ell=0).", ha="center", color=style.MUTED, fontsize=9)
    ...
    axes[1].set_title("Tip-induced on-axis modulation", color=style.TEXT, fontsize=12)
