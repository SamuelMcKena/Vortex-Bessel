...
def test_canonical_20_pixel_carrier_predicts_about_1p93_mm_shift_for_f300() -> None:
    ...
    assert run.metrics.slm_active_width_mm == 15.36
    assert run.metrics.slm_active_height_mm == 8.64
...
def test_vortex_charge_survives_candidate_route_at_generous_filter_radius() -> None:
    design = _design()
    for ell in (1, 3):
        run = run_bench_candidate(
            design,
        ...
        assert abs(run.metrics.measured_phase_winding - ell) < 0.35
